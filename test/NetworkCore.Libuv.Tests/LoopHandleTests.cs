namespace NetworkCore.Libuv.Tests
{
    ...
    public sealed class LoopHandleTests : IDisposable
    {
        ...
        void Idel2Callback(Idle handle)
        {
            this.idle2Called++;
            handle.Dispose();
        }

        void Idel1Callback(Idle handle)
        {
                ...
                || !this.idle2.IsActive)
            {
                ...
                this.idle2.Start(this.Idel2Callback);
        ...
        void CheckCallback(Check handle)
        {
            if (this.loopIteration < Iterations)
            {
                ...
                {
                    this.idle1[i].Start(this.Idel1Callback);
                    this.idles1Active++;
                }
            }
            else
        ...
        public void Handles()
        {
            ...
            this.timer.Start(TimerCallback, Timeout, Timeout);
